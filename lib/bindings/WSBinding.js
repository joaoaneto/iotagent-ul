...
function broadcast(data, attributes) {
    ...
    for(const socketId in sockets)
    {
        ...
        socket.emit("onDataUpdated", data, attributes[0]);
    }
}
    ...
    wss.on('connection', function connection(ws) {
        ...
        ws.on("publishData", function (id, data, cb) {
            ...
            for(let i = 0;i < data.length;i++){
                ...
                iotAgentLib.update(d.id, "DEVICE", config.getConfig().defaultKey,
                  [{"name": "value", "type": "String", "value": ""+d.value}], information_type, function (err) {
                    if(err){
                        cb("fail");
                    }else{
                        cb("ok");
        ...
        ws.on('updateProperties', function (id, data, cb) {
            ...
            iotAgentLib.update(id, "THING", config.getConfig().defaultKey,
              [{"name": ""+key, "type": "String", "value": ""+value}], information_type, function (err) {
                  if(err){
                      cb("fail");
                  }else{
                      cb("ok");
        ...
        ws.on('listDevices', function (cb) {
            dbUtils.findByService("knot", "THING", function (err, devices) {
                ...
                for(let i = 0;i < devices.length;i++){
                    let d = devices[i];
                    let name = "";
                    for(let j = 0;j < d.staticAttributes.length;j++){
                        if(d.staticAttributes[j].name === 'name'){
                            name = d.staticAttributes[j].value;
                        }
                    }
                    ...
                    for(let j = 0;j < d.devices.length;j++){
                        let obj = {};
                        obj.id = d.devices[j].id;
                        for(let k = 0;k < d.devices[j].staticAttributes.length;k++){
                            if(d.devices[j].staticAttributes[k].name === 'value_type'){
                                obj.value_type = d.devices[j].staticAttributes[k].value;
                            }
                            else if(d.devices[j].staticAttributes[k].name === 'unit'){
                                obj.unit = d.devices[j].staticAttributes[k].value;
                            }
                            else if(d.devices[j].staticAttributes[k].name === 'type_id'){
                                obj.type_id = d.devices[j].staticAttributes[k].value;
                            }
                            else if(d.devices[j].staticAttributes[k].name === 'name'){
                                obj.name = d.devices[j].staticAttributes[k].value;
                            }
                        }
                        schema.push(obj);
                    }
                    data.push({id: d.id, name: name, schema: schema});
                }
                cb(data);
            });
