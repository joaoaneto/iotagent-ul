...
function verifyThing(id, thing_service_path) {
...
function start(callback) {
    ...
    wss.on('connection', function connection(ws) {
        ...
        ws.on("addDevice", function (msg) {
            ...
            verifyThing(data.id, thing_service_path);
            ...
            for(let i = 0;i < data.schema.length;i++){
                ...
                device.staticAttributes.push(new Attribute("thing", "String", data.id));
        ...
        ws.on("removeDevice", function (msg) {
            const data = JSON.parse(msg);

            verifyThing(data.id, "/knot/"+data.id.replace(/-/g, '_'));

            console.log('remove received: %s from %s', msg, ws.client.conn.remoteAddress);
        });

        ws.on("updateDevice", function (msg) {
            const data = JSON.parse(msg);

            const information_type = {
                type : "DEVICE",
                service : "knot",
                subservice : "/knot/"+data.id.replace(/-/g, '_'),
                cbhost: config.iota.contextBroker.host+":"+config.iota.contextBroker.port
            };

            for(let i = 0;i < data.data.length;i++){
                let d = data.data[i];
                config.getLogger().debug(context, 'Updating device ('+data.id+') '+d.id+' with value '+d.value);
                iotAgentLib.update(d.id, [{"name": "value", "type": "String", "value": ""+d.value}], "DEVICE", information_type, null);
            }

            console.log('update received: %s from %s', msg, ws.client.conn.remoteAddress);
        });
