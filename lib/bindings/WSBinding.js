...
function start(callback) {
    ...
    wss.on('connection', function connection(ws) {
        ...
        ws.on("updateSchema", function (id, data) {
            // const data = JSON.parse(msg);
        ...
        ws.on("removeDevice", function (id) {

            verifyThing(id, "/knot/"+id.replace(/-/g, '_'));

            console.log('remove received: %s from %s', id, ws.client.conn.remoteAddress);
        });

        ws.on("publishData", function (id, data) {
            //const data = JSON.parse(msg);
            ...
            config.getLogger().debug(context, 'Updating device ('+id+') '+data.id+' with value '+data.value);
            iotAgentLib.update(data.id, [{"name": "value", "type": "String", "value": ""+data.value}], "DEVICE", information_type, null);
