...
function findOneBySubService(subservice, type, callback) {
    mongoose.model('Devices', Device.internalSchema).findOne({subservice: subservice, type: type}, function (err, device) {
        if (err) {
            callback(err, null);
        } else {
            callback(err, device);
...
exports.findOneBySubService = findOneBySubService;
